...
static int file_write(char *fn, char *first, char *last)
{
	...
	fd = open(fn, O_WRONLY | O_CREAT | O_TRUNC); /* dma: 2-arg open;
			 * xv6 has no ftruncate, so truncate up front */
	if (fd < 0)
		return -1;
	cnt = last - first + 1;
...
# endif
	...
	// dma: no getopt32 — the port takes no flags, argv[1..] are files.
	opts = 0;
	(void)opts;
	optind = 1;
	...
	/* dma: no EXINIT / ~/.exrc — no environment on this system */
