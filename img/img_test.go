...
// tests, but every cross-segment address carries a relocation. Returns the
// image and the data-segment offsets of (r, done).
func buildAddProgram(t *testing.T, a, b uint32) (*img.Image, uint32, uint32) {
	t.Helper()
	bld := img.NewBuilder()
	...
	ctrl := cfg.ExecCtrl()
	sniff := img.Abs(emu.SniffDataAddr)

	text.BlockP(img.In(data, va), sniff, 1, ctrl)
	text.BlockP(img.In(data, vb), img.In(data, bucket), 1, ctrl|emu.CtrlSniffEn)
...
func runImage(t *testing.T, im *img.Image, pl img.Placement, watch uint32) *emu.Machine {
	t.Helper()
	m := emu.NewMachine()
...
func TestTier1Load(t *testing.T) {
	im, r, done := buildAddProgram(t, 0x1111, 0x2222)
	m := runImage(t, im, nil, 0x20010000+done)
...
// behave identically wherever its segments are placed.
func TestTier2Relocation(t *testing.T) {
	...
	for name, pl := range placements {
		t.Run(name, func(t *testing.T) {
			im, r, done := buildAddProgram(t, 40000, 2345)
			dataBase := uint32(0x20010000)
			if p, ok := pl[1]; ok {
				dataBase = p
			}
			m := runImage(t, im, pl, dataBase+done)
...
func TestTier2Jump(t *testing.T) {
	...
	m := runImage(t, im, pl, 0x20038000+done)
...
func TestWriteRef(t *testing.T) {
	...
	cfg := img.DefaultMachine()
	text.BlockP(img.In(data, one), img.In(data, done), 1, cfg.ExecCtrl())
	text.Halt()
	bld.Entry(text, 0)
	...
	m := runImage(t, im, img.Placement{1: dataBase}, dataBase+done)
...
func TestOverlapRejected(t *testing.T) {
	im, _, done := buildAddProgram(t, 1, 2)
	_ = done
	m := emu.NewMachine()
