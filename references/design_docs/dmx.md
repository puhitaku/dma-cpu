...
entry point. Producers: the `img` Go package, directly and through
`dmaasm`, which builds its output image with `img.Builder`. Consumers:
`host/img/load.go` (emulator/host, the behavioural reference) and
`target/loader/dmx.c` (RP2 bare-metal).
...
| 0x18 | `entryOff` | byte offset of the entry block; multiple of 16 (8 in compact) |
...
- The resolved entry address is a multiple of the encoding's record size:
  16 for classic control blocks, 8 for compact records.
...
fetch/execute/fix channels (classic ABI v0: channels 0/1/2, scratch word
`0x2003FF00`) and writes the fetch channel's `CTRL_TRIG` last, which
begins execution. A compact-encoded image is started by arming fetch alone
— its banks and cleanup channel are configured by the image's own init
writes, and there is no fix channel or scratch word. Reference
implementations: `emu.SetupFetchExec` (Go) and `dmx_start` (C).
