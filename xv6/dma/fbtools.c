/* fbtools: the framebuffer utilities (fbtest, show) as their own
 * multi-call binary, split out of the toolbox — the fb code is a
 * third of the blob, and every exec COPIES its text into the arena,
 * so displayless boards were paying ~15 KB per toolbox instance for
 * tools they cannot link. Installed only on boards with FbBuf. */
...
main(int argc, char **argv)
{
  ...
  write(2, "usage: fbtest | show DIR|FILE...|DECK [series]\n", 47);
  exit(1);
}
